...
def nltk_metrics(hypotheses: List[str], references: List[str]) -> List[float]:
    """
    Raw corpus BLEU, GLEU, and NIST score from nltk

    :param hypotheses: list of hypotheses (strings)
    :param references: list of references (strings)
    :return: [bleu score, gleu score, nist score]
    """
    scores = [0, 0, 0]
    scores[0] = sentence_bleu([references], hypotheses)
    scores[1] = sentence_gleu([references], hypotheses)
    scores[2] = sentence_nist([references], hypotheses)
    return scores
