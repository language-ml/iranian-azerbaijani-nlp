...
from joeynmt.metrics import bleu, chrf, sequence_accuracy, token_accuracy, nltk_metrics
...
def predict(
    ...
    if data.has_trg:
        ...
        nltk_scores = nltk_metrics(valid_hyp_1best, valid_ref)
        valid_scores["nltk_bleu"] = nltk_scores[0]
        valid_scores["nltk_gleu"] = nltk_scores[1]
        valid_scores["nltk_nist"] = nltk_scores[2]

        eval_duration = time.time() - eval_start_time
        score_str = ", ".join([
            f"{eval_metric}: {valid_scores[eval_metric]:6.2f}"
            for eval_metric in eval_metrics + ["loss", "ppl", "acc", "nltk_bleu", "nltk_gleu", "nltk_nist"]
